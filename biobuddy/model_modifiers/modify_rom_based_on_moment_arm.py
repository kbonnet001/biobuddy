...
class MuscleMomentArmAnalyzer(MuscleValidator):
    ...
    def __init__(self, model_path: str, model, nb_states=50, custom_ranges=None):
        """
        ...
        """
        MuscleValidator.__init__(self, model, nb_states, custom_ranges)
    ...
    def compute_moment_arm_ranges(self, tol: float = 1e-6):
        """
        ...
        for idx_q in range(n_q):
            for idx_m in range(n_m):
                ...
                if np.all(np.abs(self.muscle_moment_arm[idx_q, idx_m]) < tol):
                    ...
                    prod = self.muscle_moment_arm[idx_q, idx_m, :-1] * self.muscle_moment_arm[idx_q, idx_m, 1:]
                    flip_idx = np.where(prod < 0)[0]

                    # 2. Newton --> find zeros more precisely
                    zeros = []
                    for i in flip_idx:
                        q_star = self.find_zero_newton(self.states[:, i], idx_q, idx_m)
    ...
    def merge_ranges_joint(self, idx_q):
        ...
        ranges = self.get_ranges_from_idx_q(self.ranges_by_joint, idx_q)
    ...
    def create_accurate_rom(self):
        ...
        for idx_q, q_name in enumerate(self.model.dof_names):
            ...
            for m_name in self.model.muscle_names:
                if self.accurate_ranges_by_joint[q_name][m_name]:
                    rom_range[0] = max(
                        rom_range[0],
                    ...
                    warnings.warn(
                        f"No accurate range found for {q_name}, {m_name}. Skipping this muscle in ROM calculation."
                    )

            self.accurate_ranges_array[idx_q, :] = rom_range
    ...
    def plot_ranges_with_true_button(self, path_to_save="", show_plot=True):
        ...
        fig = make_subplots(
            rows=nb_line,
            cols=nb_column,
            subplot_titles=[f"q{idx} - {q_name}" for idx, q_name in enumerate(self.ranges_by_joint)],
        )

        legend_added = {"pos": False, "neg": False, "zero": False}
        trace_indices_true = []

        for idx_q, q_name in enumerate(self.ranges_by_joint):
            row, col = idx_q + 1, 1
            muscles = list(self.ranges_by_joint[q_name].keys())
            component_names = list(self.ranges_by_joint[q_name].keys())

            for m_name in muscles:
                all_ranges = self.ranges_by_joint[q_name][m_name]
                true_ranges = self.accurate_ranges_by_joint.get(q_name, {}).get(m_name, [])
